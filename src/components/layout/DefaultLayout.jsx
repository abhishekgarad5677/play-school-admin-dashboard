...
export default function DefaultLayout() {
  ...
  const acticeTabStyle = { backgroundColor: "#5d87ff", color: "#fff" };
  const acticeIconStyle = { color: "#fff" };
  ...
  // ✅ Sidebar items: super sees all, others see only their permitted routes
  const visibleSidebarRoutes = isSuper
    ? SideBarRoutes.filter((r) => r.showInSidebar)
    : SideBarRoutes.filter(
        (r) => r.showInSidebar && permissions.includes(r.path),
      );
    ...
    <ThemeProvider theme={isDarkMode ? darkTheme : lightTheme}>
      <Box sx={{ display: "flex" }}>
        ...
        >
          ...
          <List>
            {visibleSidebarRoutes.map((ele, index) => (
              <ListItem key={index} disablePadding sx={{ display: "block" }}>
                <ListItemButton
                  component={Link}
                  to={ele.path}
              ...
              ({
                ...
                <Route
                  ...
                  element={
                    ...
                    >
                      <PageComponent />{" "}
                      {/* ✅ instantiated here, not in the config */}
                    </PermissionRoute>
                  }
                />
              ),
